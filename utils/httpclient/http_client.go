...
func httpCall(ctx context.Context, method string, url string, data Map, config HttpConfig, response interface{}) error {
	...
	if err != nil {
		utils.Errormf(ctx, Map{"eventName": constants.ErrorCode_Http_Error, "method": strings.ToLower(method), "url": url, "error": fmt.Sprintf("%v", err)}, "Error reading response body - reason: %v", err)
		return err
	}
	if res.StatusCode >= 400 {
		responseString := string(responseBytes)
		utils.Errormf(ctx, Map{"eventName": constants.ErrorCode_Http_Error, "method": strings.ToLower(method), "url": url, "error": responseString}, "Request resource returned an error %v - response: %v", res.StatusCode, responseString)
